...
connection.listen({
	...
	onTextMessage: function(message) {
		...
		if (currentLi.className != 'clearfix active') {
			...
			var str =
				`<div class="other-content fl clearfix">
              <div class="avatar fl">
...
function addFriendListEvent() {
	...
	for (var i = 0; i < lis.length; i++) {
		...
		lis[i].onclick = function() {
			...
			// 获取激活类下面的文本值
			var user_name = this.getElementsByClassName('user-name')[0].textContent;
			// 给标题赋值
			document.getElementsByClassName('title')[0].textContent = user_name;
            ...
            <div class="fl bubble l-bubble">
              ...
              ${msgData[j].msg}
							...
							`<div class="other-content fr clearfix">
	...
	//显示黑名单
	$('#innerbox>li').on('contextmenu', function(e) {
...
window.onload = function() {
	...
	$('#close').on('click', function() {
		...
		//显示聊天面板
		getId('chat-box').style.display = 'none';
	...
	//回车发送消息
	getId('input-content').onkeydown = function(e) {
		if (e.keyCode == 13) {
			myChar();
		}
	}
	...
	// 右键菜单
	$('.blacklist li').on('click', function() {
		// console.log(friendId);
		menuState = $(this).attr('name');
		if (menuState == 'delete') {
			// 删除好友
			connection.removeRoster({
				...
				success: function() { // 删除成功
					...
					// 还不能自动刷新
					//更新我的好友列表
					var li = createFriedList(friendId);
					//将li添加到 ul.innerbox
					getId('innerbox').appendChild(li);
				},
				error: function() { // 删除失败
